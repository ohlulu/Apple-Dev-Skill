...
| Content overflow detection. Two-pass layout — show everything, then measure actual wrap; never estimate widths. | [overflow-detection](references/overflow-detection.md) |
| Shadow and clipping. Ancestor clipsToBounds chain, shadow + cornerRadius parent/child split, layer frame ownership, UILabel background trap, iOS 26 sheet color seam. | [shadow-and-clipping](references/shadow-and-clipping.md) |
| Keyboard avoidance. Scroll-view inset adjustment via keyboardWillChangeFrame, animation sync, inputView handling, first-responder scrolling. | [keyboard-avoidance](references/keyboard-avoidance.md) |
