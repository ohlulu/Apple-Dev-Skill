...
| Custom animated-image playback in UIImageView subclasses. The `isAnimating` override that silently blocks layer-contents writes, UICollectionView hidden-pooling vs visibility-driven pause (no un-hide hook), and the probe ladder for "frames delivered but screen frozen". | [animated-image-playback](references/animated-image-playback.md) |
