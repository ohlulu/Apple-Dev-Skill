...
| Diffable data source (`Hashable` not `Identifiable` is the constraint, two Apple-blessed identifier patterns — whole model vs ID + cellProvider lookup, `reconfigureItems` as the official content-update channel, iOS 15 `animatingDifferences:false` semantic change + `applySnapshotUsingReloadData`, background-thread snapshot assembly with single-thread discipline, common mistakes when sharing models between UIKit Diffable and SwiftUI `ForEach`) | [diffable-data-source](references/diffable-data-source.md) |
...
| iOS 26 behavior changes catalog (consolidated index of deltas from iOS 18: Liquid Glass sidebar overlay, primaryBackgroundStyle vs primaryBackgroundEffect, setViewController .zero-frame regression, pushViewController same regression fixed in 26.2, push transition full-width slide, UISearchController nav-bar capsule tint is no longer reachable from app code under Liquid Glass; what did NOT change but was assumed to; diagnostic workflow when suspecting an iOS 26 regression) | [ios-26-behavior-changes](references/ios-26-behavior-changes.md) |
...
| Navigation bar appearance (UINavigationBarAppearance slots, iOS 26 backgroundColor breaking change, Liquid Glass compatibility, three-slot consistency, large title collapse scroll tracking, alwaysBounceVertical, split appearance for opaque chrome, embedded UISearchBar independent background, iOS 26 prescription — abandon UISearchController for any list screen that needs a custom search-field tint, host a custom search-pill field in the root view instead) | [navigation-bar-appearance](references/navigation-bar-appearance.md) |
...
| iCloud ubiquity container (NSMetadataQuery discovery, startDownloadingUbiquitousItem, progress-based timeout, bulk download, ubiquitousItemDownloadingErrorKey, NSUbiquitousContainers Files app visibility, fresh-install restore; upload-confirmation wait — fresh `URL` per poll because URL caches resourceValues so `isUploaded`/downloadingStatus never flips on stale instances, progress from the upload poll not the local copy loop, stall/liveness detection via `isUploading` with budget floor, per-file state dump on failure) | [icloud-ubiquity](references/icloud-ubiquity.md) |
| Cloud backup providers / OAuth REST (Google Drive / Dropbox versioned backup: user-visible storage as the default — `drive.file` + `My Drive/<App>/` over hidden `appDataFolder`; OAuth scope-expansion 403 `insufficientScopes` → wipe access+refresh tokens → full re-consent because a refresh keeps the old scope set; `URLSession.upload(for:from:)` body via `from:` only, never also `httpBody`; atomic publish via a single pointer file committed last + millisecond-precision version ids; pointer NAMESPACES for breaking bundle formats — `CURRENT.json` only ever references bundles the oldest shipped reader can decode, newer schemas commit to `CURRENT2.json`, readers resolve newest across namespaces, cleanup keeps every namespace target; idempotent best-effort storage-location migration hook run on restore with old data left as a safety net; progress/completion reflect the durable commit not a faked 1.0 with late-callback guard; time-boxed cleanup that never blocks completion; restore-integrity hardening — basename path-traversal guard, SHA-256 resume skip, write gate around the swap, close live DB connections before swapping the sqlite file (vnode-unlinked-while-in-use API violation); dedicated ephemeral transfer session with URLCache fully disabled — CFNetwork caches POST-download responses and cached-POST replay dies with -1005 errno 57 while first fetch succeeds; bounded -1005 retry because URLSession auto-retries only idempotent GETs never POSTs; curl-replay with the same bearer token as the client-vs-server discriminating diagnostic) | [cloud-backup-providers](references/cloud-backup-providers.md) |
